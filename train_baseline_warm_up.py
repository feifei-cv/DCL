...
from dataset.cityscapes_pair_dataset import cityscapesDataSet
...
def main():
    ...
    ## train
    interp = nn.Upsample(size=(args.input_size, args.input_size), mode='bilinear', align_corners=True) ## 512
    interp_target = nn.Upsample(size=(args.input_size_target, args.input_size_target), mode='bilinear', align_corners=True) ##960
    sys.stdout = Logger('logs/train_log.txt')  ### log record
    ...
    for i_iter in range(args.num_steps):
        ...
        for sub_i in range(args.iter_size):
            ...
            if args.model == 'RefineNet':
                pred_target_d_t, feature_d_t = model(images_d)
                pred_target_n_t, feature_n_t = model(images_n)
            else:
                _, pred_target_d_t, feature_d_t = model(images_d)
                _, pred_target_n_t, feature_n_t = model(images_n)
            pred_target_d = interp_target(pred_target_d_t)
            pred_target_n = interp_target(pred_target_n_t)
            ...
            loss = 0.01 * loss_adv_target_n_19 + loss_pseudo  ##对抗损失+light loss
            loss_adv2 = loss / args.iter_size
            # loss.backward()  #
            loss_adv_target_value += loss_adv_target_n_19.item() / args.iter_size

            # train with source
            _, batch = trainloader_iter.__next__()
            images, labels, images_s_n, labels_s_n, _, _ = batch
            images, images_s_n = images.to(device), images_s_n.to(device)
            labels, labels_s_n = labels.long().to(device), labels_s_n.long().to(device)

            if args.model == 'RefineNet':
                pred_c_s, feature_d_s= model(images)
                pred_c_n_s, feature_n_s = model(images_s_n)
            else:
                _, pred_c_s, feature_d_s = model(images)
                _, pred_c_n_s, feature_n_s = model(images_s_n)
            pred_c = interp(pred_c_s)
            pred_c_n = interp(pred_c_n_s)

            ### seg
            loss_seg = seg_loss(pred_c, labels) + seg_loss(pred_c_n, labels_s_n) + loss_adv1 + loss_adv2
            ### total loss
            loss = loss_seg  + loss_pseudo
        ...
        optimizer_D1.step()
        optimizer_D2.step()
        print('iter = {0:8d}/{1:8d}, loss_seg = {2:.3f}, loss_adv = {3:.3f}, loss_D1 = {4:.3f}, loss_D2 = {5:.3f}, loss_pseudo = {6:.3f}'.format(
                i_iter, args.num_steps, loss_seg_value,
                loss_adv_target_value, loss_D_value1, loss_D_value2, loss_pseudo))

        # print('iter = {0:1d}/{1:5d}, loss_seg = {2:.3f}, loss_delta = {3:.3f}, '
        #     'loss_pseudo = {4:.3f}, loss_iwc = {5:.3f}'.format(i_iter, args.num_steps,
        #                                     loss_seg_value, loss_delta, loss_pseudo, iwc_loss))
        if i_iter % args.save_pred_every == 0 and i_iter != 0:
            print('taking snapshot ...')
            torch.save(model.state_dict(), os.path.join(args.snapshot_dir, 'DCL' + str(i_iter) + '.pth'))
